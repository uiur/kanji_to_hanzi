...
class KanjiToHanzi
  ...
  def self.load_character_table
    ...
    # Build one-to-one mapping only
    @reverse_simplified_table = {}
    @reverse_traditional_table = {}

    data.split("\n")
      .map { |line|
        japanese, traditional, simplified = line.split("\t")
        [japanese, traditional.split(','), simplified.split(',')]
      }
      # Prioritize the kanji with one-to-one mapping
      .sort_by { |japanese, traditional, simplified| traditional.size + simplified.size }
      .each do |japanese, traditional, simplified|
        next unless japanese =~ REGULAR_USE_KANJI_REGEXP

        simplified.each do |s|
          next if s == 'N/A'
          next if s == japanese
          next if s =~ REGULAR_USE_KANJI_REGEXP
          next if @reverse_simplified_table[s]
          @reverse_simplified_table[s] = [japanese]
        end

        traditional.each do |t|
          next if t == 'N/A'
          next if t == japanese
          next if t =~ REGULAR_USE_KANJI_REGEXP
          next if @reverse_traditional_table[t]
          @reverse_traditional_table[t] = [japanese]
        end
